<?php
// jika data settings sudah ada maka update data tersebut
...
if (isset($_POST['simpan'])) {
    ...
    if ($row) {
        ...
        $update = mysqli_query($koneksi, "UPDATE settings SET email = '$email', phone = '$phone', alamat = '$alamat', instagram = '$instagram', facebook = '$facebook', twitter = '$twitter', linkedin = '$linkedin', logo = '$logo_name' WHERE id = '$id_settings'");
        if ($update) {
            header("location:?page=settings&ubah=berhasil");
        }
    } else {
        $insert = mysqli_query($koneksi, "INSERT INTO settings (email, phone, alamat, instagram, facebook, twitter, linkedin, logo) VALUES ('$email', '$phone', '$alamat', 'instagram', 'facebook', 'twitter', 'linkedin', 'logo')");
        if ($insert) {
